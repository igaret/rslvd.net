...
router.post('/', async (req, res) => {
  try {
    ...
    if (parent_id) {
      ...
      // Nested FQDNs aren't covered by the *.rslvd.net wildcard cert, so
      // auto-issue a dedicated cert + nginx block in the background.
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    // Clean up any per-tunnel cert + nginx block (no-op for single-level)
